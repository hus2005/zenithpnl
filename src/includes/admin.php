...
$_ = getUserLang(MAIN_HOME . 'includes/langs/');
$allowedLangs = getAvailableLanguages(MAIN_HOME . 'includes/langs/');
 ...
 * 
 * Scans the specified directory for INI files and returns an array
 * of available language codes. Each language code corresponds to
 * a valid and readable INI file in the directory.
 *
 * @param string $langsDir Path to the languages directory
 * @return array Array of available language codes
 */
function getAvailableLanguages($langsDir) {
    $allowedLangs = [];

    // Get all INI files from the languages directory
    $files = glob($langsDir . '*.ini');

    // Extract language codes from filenames
    foreach ($files as $file) {
        // Verify it's a valid file before including
        if (is_file($file) && is_readable($file)) {
            $langCode = pathinfo($file, PATHINFO_FILENAME);
            $allowedLangs[] = $langCode;
        }
    }

    // Fallback to English if no language files are found
...
/**
 * Get user language strings from INI file based on cookie preference
 * 
 * Loads the appropriate language file based on the 'lang' cookie value.
 * Falls back to English if the requested language is not available, 
 * no cookie is set, or file parsing fails.
 *
 * @param string $langsDir Path to the languages directory
 * @return array Associative array of language strings from the INI file
 *               Returns empty array if all fallbacks fail
 */
function getUserLang($langsDir) {
    // Get available languages
    $allowedLangs = getAvailableLanguages($langsDir);

    // Get language from cookie or default to English
    $langCode = $_COOKIE['lang'] ?? 'en';
    
    // Validate and sanitize language code
    $langCode = in_array($langCode, $allowedLangs) ? $langCode : 'en';

    // Load language strings with error handling
    $langFile = $langsDir . $langCode . '.ini';
    $langStrings = parse_ini_file($langFile);
    
    // Final fallback if parsing fails
    if ($langStrings === false) {
        $fallbackFile = $langsDir . 'en.ini';
        $langStrings = parse_ini_file($fallbackFile) ?: [];
    }
